...
#os.environ["XLA_PYTHON_CLIENT_MEM_FRACTION"]=".16"
...
flags.DEFINE_integer('model_seed', 28, 'Seed model initialization.')
...
flags.DEFINE_integer('train_items', 40000,
...
flags.DEFINE_string('log_param_count', 'True', 'Whether to log number of model parameters')
...
#  print("训练边界：", FLAGS.train_items)
  while current_train_items < FLAGS.train_items:
#    print("当前的item：", current_train_items)
...
#    print("step:", step)
    ...
    if current_train_items >= next_eval:
      ...
      if FLAGS.eval_on_train_set:
        ...
        if train_stats['loss'] < min_loss:
            ...
            train_model.save_model("best1.pkl")
  ...
  if FLAGS.eval_on_test_set:
    ...
    test_stats3 = collect_and_eval(
        ...
        spec=spec,
        extras=common_extras)
    eval_model.restore_model('best1.pkl', only_load_processor=False)
    test_stats2 = collect_and_eval(
        test_sampler,
        eval_model.predict,
        test_samples,
        rng_key,
        ...
        extras=common_extras)
    rng_key = new_rng_key
    logging.info('(test) step %d: %s', step, test_stats1)
    logging.info('(best from loss): %s', test_stats2)
    logging.info('(last): %s', test_stats3)
#    save_stats(test_stats1['score'], 'test_stats.pkl')
...
def main1(unused_argv):
  ...
  for alg in specs.CLRS_30_ALGS:
  	start_time = time.time()
  	logging.info("alg: %s", alg)
  	main(alg)
  	end_time = time.time()
  	tmp = (end_time-start_time)//60
  	logging.info("%s运行时间约：%d min.\n", alg, tmp)
  	whole_time += tmp
  	logging.info("当前总运行时间约：%d min.", whole_time)
