...
class RT(Processor):
    ...
    def __call__(
        ...
        alpha_mlp1 = hk.Linear(1)
        alpha = jax.nn.sigmoid(alpha_mlp1(bank[0][0]))
        # alpha = jax.nn.relu(jax.nn.tanh(alpha_mlp1(bank[0][0])))
        old = jnp.multiply(bank[0][0], alpha)
        reversed_alpha = jnp.subtract(1, alpha)
        new = jnp.multiply(node_tensors, reversed_alpha)
        bank[0] = bank[0].at[0, :, :, :].set(old + new)

        alpha_mlp2 = hk.Linear(1)
        alpha = jax.nn.sigmoid(alpha_mlp2(bank[1][0]))
        # alpha = jax.nn.relu(jax.nn.tanh(alpha_mlp2(bank[1][0])))
        old = jnp.multiply(bank[1][0], alpha)
        reversed_alpha = jnp.subtract(1, alpha)
        new = jnp.multiply(edge_tensors, reversed_alpha)
        bank[1] = bank[1].at[0, :, :, :, :].set(old + new)
        ...
        for layer in layers:
            node_tensors, edge_tensors = layer(node_tensors, edge_tensors, graph_tensors, adj_mat, hidden, bank=bank, step=step)
        ...
        return out_nodes, out_edges, out_graph if self.graph_vec == 'core' else None, bank
