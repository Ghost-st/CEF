...
class Net(hk.Module):
  ...
  def _msg_passing_step(self,
    ...
    (n_hiddens, e_hiddens, g_hiddens, output_preds_cand, hint_preds, diff_logits,
     lstm_state, bank) = self._one_step_pred(inputs, cur_hint,
                                       mp_state.n_hiddens, mp_state.e_hiddens, mp_state.g_hiddens,
                                       batch_size, nb_nodes,
                                       mp_state.lstm_state,
                                       spec, encs, decs, diff_decs, mp_state.bank, i)
  ...
  def __call__(self, features_list: List[_Features], repred: bool,
    ...
    for algorithm_index, features in zip(algorithm_indices, features_list):
      ...
      bank1 = jnp.zeros((1, batch_size, nb_nodes, self.hidden_dim))
      bank2 = jnp.zeros((1, batch_size, nb_nodes, nb_nodes, self.hidden_dim))
#      bank3 = jnp.zeros((1, batch_size, nb_nodes, self.hidden_dim))
      bank = [bank1, bank2]
  ...
  def _one_step_pred(
    ...
    # PROCESS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    nxt_n_hidden, nxt_e_hidden, nxt_g_hidden, bank = self.processor(
        node_fts,
        edge_fts,
        graph_fts,
        adj_mat,
    ...
    return nxt_n_hidden, nxt_e_hidden, nxt_g_hidden, output_preds, hint_preds, diff_preds, nxt_lstm_state, bank
